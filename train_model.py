    ...
    try:
        with open(file_path, "r") as file:
            for line in file:
                ...
                training_data.append((cpf, [(start, end, "CPF")]))
        print(f"[INFO] Dados de treinamento carregados de {file_path}")
    except FileNotFoundError:
        print(f"[ERRO] Arquivo {file_path} não encontrado!")
    except Exception as e:
        print(f"[ERRO] Ocorreu um erro ao carregar os dados de treinamento: {e}")
...
def train_model(training_data, model_dir="models/ner_model", epochs=10):
    ...
    if os.path.exists(model_dir):
        print(f"[INFO] Diretório '{model_dir}' já existe. Carregando modelo existente...")
...
def evaluate_model(nlp, test_file="models/cpf_data.txt"):
    ...
    for text, annotations in test_data:
        ...
        actual_cpf = [cpf for _, cpf in annotations]
...
def detect_cpf(text, model_dir="models/ner_model"):
    # Verificar se o diretório do modelo existe
    if not os.path.exists(model_dir):
        print(f"[ERRO] O diretório '{model_dir}' não existe! Carregue um modelo treinado primeiro.")
        return
    ...
    for ent in doc.ents:
        if ent.label_ == "CPF":
            print(f"CPF detectado: {ent.text} (Posição: {ent.start_char}-{ent.end_char})")
